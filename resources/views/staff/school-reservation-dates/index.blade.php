...
@section('title', 'Date di Ritiro Prenotazioni')

@section('content')
    <!-- Header -->
    <div class="mb-8">
        <h1 class="text-3xl font-bold text-gray-900">Date di Ritiro Prenotazioni</h1>
        <p class="text-gray-600 mt-2">Configura le date disponibili per il ritiro dei libri prenotati online</p>
    </div>
            ...
            <div class="col-span-3">
                <div class="bg-white rounded-lg shadow-sm border border-gray-200 p-8">
                    <div class="flex items-center justify-between mb-6">
                        ...
                        <a href="{{ route('staff.reservation-dates.create') }}" class="px-6 py-2 bg-blue-600 text-white font-medium rounded-lg hover:bg-blue-700 transition">
                            Aggiungi Data
                        </a>
                    </div>
                            ...
                            @foreach ($activeDates as $date)
                                <div class="bg-gradient-to-br from-green-50 to-emerald-50 border-2 border-green-200 rounded-lg p-5 hover:shadow-lg transition">
                                    <div class="flex items-start justify-between mb-3">
                                        <div class="flex-1">
                                            <p class="text-sm font-semibold text-green-900">{{ $date->label ?? 'Ritiro prenotazioni' }}</p>
                                    ...
                                    <div class="flex gap-2 pt-3 border-t border-green-200">
                                        <a href="{{ route('staff.reservation-dates.edit', $date) }}" class="flex-1 px-3 py-2 text-center text-sm font-medium text-green-700 bg-white border border-green-300 rounded-lg hover:bg-green-50 transition">
                                            Modifica
                                        </a>
                                        <form action="{{ route('staff.reservation-dates.toggle', $date) }}" method="POST" class="flex-1">
                                            @csrf
                                            @method('PATCH')
                                        ...
                                        <form action="{{ route('staff.reservation-dates.destroy', $date) }}" method="POST" class="flex-1" onsubmit="return confirm('Elimina questa data?');">
                                            @csrf
                                            @method('DELETE')
                    ...
                    @else
                        <div class="bg-blue-50 border-2 border-blue-300 rounded-lg p-8 text-center">
                            ...
                            <p class="text-blue-800 font-medium">Nessuna data di ritiro prenotazioni attiva</p>
                            <p class="text-sm text-blue-600 mt-1">Aggiungi una data per permettere agli studenti di prenotare il ritiro dei libri</p>
                        </div>
                    @endif
                            ...
                            <div class="grid grid-cols-1 gap-4">
                                @foreach ($inactiveDates as $date)
                                    <div class="bg-gradient-to-br from-gray-50 to-slate-50 border-2 border-gray-300 rounded-lg p-5 opacity-75 hover:shadow-lg transition">
                                        <div class="flex items-start justify-between mb-3">
                                            <div class="flex-1">
                                                <p class="text-sm font-semibold text-gray-600">{{ $date->label ?? 'Ritiro prenotazioni' }}</p>
                                        ...
                                        <div class="flex gap-2 pt-3 border-t border-gray-300">
                                            <a href="{{ route('staff.reservation-dates.edit', $date) }}" class="flex-1 px-3 py-2 text-center text-sm font-medium text-gray-700 bg-white border border-gray-300 rounded-lg hover:bg-gray-50 transition">
                                                Modifica
                                            </a>
                                            <form action="{{ route('staff.reservation-dates.toggle', $date) }}" method="POST" class="flex-1">
                                                @csrf
