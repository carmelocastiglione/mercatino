...
@section('title', 'Riepilogo Vendite')
...
@section('content')
    <div class="max-w-4xl mx-auto">
        ...
        <!-- Main Content -->
        <div class="space-y-8">
            <x-batch-information-card :batch="$batch" />

            <!-- Login Credentials Box (shown only if school has online sales and credentials are in session) -->
            @php
                $buyerEmail = $batch->buyer->email;
                $buyerPassword = session()->pull('buyer_login_credentials.password');
                $enableOnlineSales = auth()->user()->school->getSetting('enable_online_sales');
            @endphp
            
            @if($enableOnlineSales && $buyerPassword)
                    ...
                    <div class="grid grid-cols-1 md:grid-cols-2 gap-6 print-credentials-grid">
                        <div class="text-center flex flex-col items-center">
                            ...
                            <div class="bg-white border-2 border-green-300 rounded-lg p-4 w-full flex items-center justify-center">
                                <p class="font-mono text-lg text-green-700 break-all">{{ $buyerEmail }}</p>
                            </div>
                        </div>
                        ...
                        <div class="text-center flex flex-col items-center">
                            ...
                            <div class="bg-white border-2 border-green-300 rounded-lg p-4 w-full flex items-center justify-center">
                                <code id="password_field" class="font-mono text-lg text-green-700">{{ $buyerPassword }}</code>
                            </div>
                        </div>
                    </div>
                </div>
            ...
            <x-sales-books-table :batch="$batch" />

            <x-summary-footer :saleBatch="$batch" />

            <x-information-note message="La vendita è stata registrata correttamente. Stampa questo riepilogo e conserva una copia come documentazione ufficiale della transazione." />

            <div class="flex gap-4 print:hidden">
                <a href="{{ route('staff.sales.index') }}" class="flex-1 px-6 py-4 bg-gray-600 text-white font-medium rounded-lg hover:bg-gray-700 transition text-center">
                    Torna alle vendite
                </a>
