...
@section('title', isset($isBatch) && $isBatch ? 'Riepilogo Vendite' : 'Riepilogo Vendita')
...
@section('content')
    <div class="max-w-4xl mx-auto">
        ...
        <div class="mb-8 flex items-center justify-between">
            <div>
                <h1 class="text-4xl font-bold text-gray-900">{{ isset($isBatch) && $isBatch ? 'Riepilogo Vendite' : 'Riepilogo Vendita' }}</h1>
                <p class="text-gray-600 mt-2">
                    @if(isset($isBatch) && $isBatch)
                        {{ count($sales) }} vendita/e registrate - Data: {{ $sales->first()->created_at->format('d/m/Y H:i') }}
                    @else
        ...
        <div class="space-y-8">
            ...
            @if(isset($isBatch) && $isBatch)
                <!-- Summary Stats -->
                <div class="bg-gradient-to-r from-green-50 to-green-100 border-2 border-green-300 rounded-lg p-6 shadow-md">
                    <div class="grid grid-cols-1 md:grid-cols-3 gap-6">
                        <div class="text-center">
                            <p class="text-sm text-gray-600 mb-2">Numero Vendite</p>
                            <p class="text-4xl font-bold text-green-600">{{ count($sales) }}</p>
                        </div>
                        <div class="text-center">
                            <p class="text-sm text-gray-600 mb-2">Importo Totale</p>
                            <p class="text-4xl font-bold text-green-600">€{{ number_format($sales->sum(fn($s) => $s->bookListing->price), 2, ',', '.') }}</p>
                        </div>
                        <div class="text-center">
                            <p class="text-sm text-gray-600 mb-2">Ora Registrazione</p>
                            <p class="text-lg font-bold text-green-600">{{ $sales->first()->created_at->format('H:i:s') }}</p>
                        </div>
                    </div>
                </div>
                ...
                <div class="bg-white rounded-lg shadow-sm border border-gray-200 overflow-hidden">
                    <div class="px-8 py-6 border-b border-gray-200 bg-gray-50">
                        <h2 class="text-2xl font-bold text-gray-900">Libri Venduti</h2>
                    </div>
                        ...
                        <table class="w-full">
                            <thead class="bg-gray-50 border-b border-gray-200">
                                <tr>
                                    ...
                                    <th class="px-6 py-4 text-left text-sm font-semibold text-gray-900">Autore</th>
                                    <th class="px-6 py-4 text-left text-sm font-semibold text-gray-900">Condizione</th>
                                    <th class="px-6 py-4 text-left text-sm font-semibold text-gray-900">Acquirente</th>
                                    <th class="px-6 py-4 text-right text-sm font-semibold text-gray-900">Prezzo</th>
                                </tr>
                            </thead>
                            <tbody class="divide-y divide-gray-200">
                                @foreach($sales as $batchSale)
                                    <tr class="hover:bg-gray-50 transition">
                                        <td class="px-6 py-4 text-sm text-gray-900 font-medium">
                                            {{ $batchSale->bookListing->book->title }}
                                        </td>
                                        <td class="px-6 py-4 text-sm text-gray-600">
                                            {{ $batchSale->bookListing->book->author }}
                                        </td>
                                        <td class="px-6 py-4 text-sm">
                                            <span class="px-3 py-1 rounded-full text-xs font-semibold
                                                @switch($batchSale->bookListing->condition)
                                                    @case('like-new')
                                                        bg-green-100 text-green-800
                                                        @break
                                                    @case('good')
                                        ...
                                        <td class="px-6 py-4 text-sm text-gray-600">
                                            <p class="font-medium">{{ $batchSale->buyer->name }} {{ $batchSale->buyer->surname }}</p>
                                            <p class="text-xs text-gray-500">{{ $batchSale->buyer->code }}</p>
                                        </td>
                                        <td class="px-6 py-4 text-sm font-semibold text-gray-900 text-right">
                                            €{{ number_format($batchSale->bookListing->price, 2) }}
                                        </td>
                                    </tr>
                                @endforeach
                            </tbody>
                            <tfoot class="bg-gray-50 font-bold border-t border-gray-200">
                                <tr>
                                    <td colspan="4" class="px-6 py-4 text-right text-sm">TOTALE:</td>
                                    <td class="px-6 py-4 text-right text-sm text-green-600">
                                        €{{ number_format($sales->sum(fn($s) => $s->bookListing->price), 2, ',', '.') }}
                                    </td>
                                </tr>
                            </tfoot>
                        </table>
