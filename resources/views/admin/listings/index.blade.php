...
@section('dashboard-content')
    ...
    <div class="mb-8 flex items-center justify-between">
        <div>
            <h1 class="text-4xl font-bold text-gray-900 mb-2">Gestione Annunci</h1>
            <p class="text-gray-600">Amministra gli annunci e le copie dei libri in vendita</p>
        </div>
        <a href="{{ route('admin.listings.create') }}" class="bg-blue-600 hover:bg-blue-700 text-white font-medium py-3 px-6 rounded-lg transition">
            + Nuovo Annuncio
        </a>
    </div>
                    ...
                    @forelse ($listings as $listing)
                        <tr class="hover:bg-gray-50 transition">
                            <td class="px-6 py-4">
                                ...
                                <p class="text-xs text-gray-500 mt-1">{{ $listing->book->subject }} • {{ $listing->book->school_class }}</p>
                            </td>
                            ...
                            <td class="px-6 py-4 text-right space-x-2">
                                <a href="{{ route('admin.listings.edit', $listing) }}" class="text-blue-600 hover:text-blue-900 font-medium text-sm">Modifica</a>
                                <form method="POST" action="{{ route('admin.listings.delete', $listing) }}" class="inline" onsubmit="return confirm('Sei sicuro di voler eliminare questo annuncio?');">
                                    @csrf
                                    @method('DELETE')
                    ...
                    @empty
                        <tr>
                            <td colspan="6" class="px-6 py-12 text-center text-gray-500">
                                <p class="text-lg">Nessun annuncio trovato</p>
                                <p class="text-sm mt-1">
                                    <a href="{{ route('admin.listings.create') }}" class="text-blue-600 hover:text-blue-900 font-medium">Crea il primo annuncio</a>
                                </p>
                            </td>
                        </tr>
                    @endforelse
