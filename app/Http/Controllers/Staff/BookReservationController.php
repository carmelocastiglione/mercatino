...
class BookReservationController extends Controller
{
    ...
    /**
     * Create sales from approved reservations (JSON API endpoint)
     */
    public function createSalesBulk(Request $request): JsonResponse
    {
        try {
            $studentId = $request->input('student_id');
            $student = User::findOrFail($studentId);
            ...
            if ($confirmedReservations->isEmpty()) {
                return response()->json(['success' => false, 'message' => 'Nessuna prenotazione approvata'], 400);
            }
            ...
            foreach ($confirmedReservations as $reservation) {
                ...
                // Track which batches need status update
                $batchId = $reservation->book_reservation_batch_id;
            ...
            // Update batch statuses to confirmed
            foreach ($batchIds as $batchId) {
                BookReservationBatch::findOrFail($batchId)->update([
                    'status' => 'confirmed',
                    'confirmed_at' => now(),
                ]);
            }
            ...
            return response()->json([
                'success' => true,
                'message' => "$salesCount vendite create",
                'sales_count' => $salesCount
            ]);
        } catch (\Exception $e) {
            return response()->json([
                'success' => false,
                'message' => 'Errore nella creazione delle vendite: ' . $e->getMessage()
            ], 500);
        }
    }
